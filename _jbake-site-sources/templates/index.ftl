<#include "header.ftl">
	
	<#include "menu.ftl">

	 <!-- Marketing content -->
	 <div class="jumbotron">
	 	<div class="container">
	 		<div class="col-md-3">
	 			<img src="img/logo_palm01.png"/>
	 		</div>
	 		<div class="col-md-9">
        		<h1>Jerkar</h1>
        		<h3 class="center">Build your Java project easier, stronger, faster.</h3> 
        	</div>
        </div>
        <br/>
        
        <p class="lead">Write your build definitions directly in <strong>Java</strong> (instead of XML or script language) 
        		and leverage directly the power, robustness and simplicity of the language.<p/>
        <p class="lead">This brings unrivaled <strong>ease</strong>, <strong>flexibility</strong>, 
        		<strong>solidity</strong> and <strong>reusability</strong> for building your projects.</p>
        <a class="btn btn-large btn-success" href="tell-me-more.html">Tell me more</a>
        <a class="btn btn-large btn-success" href="binaries/jerkar-distrib.zip">Download</a>
        <p></p>
        <div class="alert alert-info">
			<strong>Latest News: </strong>Ready for experimental use</a>
		</div>
      </div>
      
      <div class="page-header">
			
	  </div>
	  
	  <img src="img/Under_construction_pointer.jpg" height="100" width="200"/>
	
<!--	<#include "sitemap.ftl"> -->

<#include "footer.ftl">
