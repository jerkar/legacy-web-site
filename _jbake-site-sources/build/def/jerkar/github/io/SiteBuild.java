...
class SiteBuild extends JkBuild {
	...
	JkFileTree jerkarCoreDocDir = JkFileTree
			.of(file("../../jerkar/org.jerkar.core/src/main/doc"));
	JkFileTree jerkarDistJavadoc = JkFileTree
			.of(file("../../jerkar/org.jerkar.distrib-all/build/output/javadoc-all"));
	File jerkarDistZip = siteBase
			.file("../jerkar/org.jerkar.distrib-all/build/output/jerkar-distrib.zip");
	...
	@JkDoc({
			"Generates the site and imports documentation inside.",
	...
	public void importDocFromJerkarProject() {
		...
		List<File> files = jerkarCoreDocDir.include("**/*.md")
				.exclude("reference/**/*").files(false);
		File temp = JkUtilsFile.tempFile("reference.md");
		jerkarCoreDocDir.from("reference").mergeTo(temp);
		files.add(temp);
		for (File file : files) {
			String relativePath = JkUtilsFile.isAncestor(
					jerkarCoreDocDir.root(), file) ? jerkarCoreDocDir
					.relativePath(file) : file.getName();
			File copied = targetDocDir.file(relativePath);
			JkLog.startln("Importing doc file " + file + " to "
					+ copied.getPath());
	...
	public void importSiteDoc() {
		...
		contentSource.copyReplacingTokens(dest,
				JkUtilsIterable.mapOf("jerkarVersion", currentJerkarVersion()));
	}

	public void copyCurrentDist() {
		JkUtilsFile.copyFileToDir(jerkarDistZip, siteDistDir,
				JkLog.infoStream());
	...
	public void copyCurrentJavadoc() {
		JkUtilsFile.copyDirContent(jerkarDistJavadoc.root(),
				siteBase.file("javadoc/latest"), false);
	}

	public void jbake() {
		JkJavaProcess
				.of()
				.withClasspath(jbakeDir.include("lib.*.jar"))
	...
	private void addMenu() {
		for (File file : menuAddedFiles) {
			...
			JkUtilsFile.writeString(file,
					ImplicitMenu.endDivHtml("end of wrapper div"), true);
		}
	}
